...
export class ReportsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.items = [{"name":"Coincidentes"},{"name":"Faltantes"},{"name":"Sobrantes"}]
  }
   updateContent(e){
    if(e){
...
analice(){
  ...
  if(this.isExcel){
     ...
     this.data = this.converter.converterExcelAreasToJsonAreasReport(this.data);
     let newData = [];
...
convertedData(data){
  ...
  for (var i = 1; i <= pageCount; i++) {
    doc.setPage(i)
    doc.text('Página ' + String(i) + ' de ' + String(pageCount), doc.internal.pageSize.width / 2, doc.internal.pageSize.getHeight()-5, {
      align: 'center'
    })
  }
  return doc;
...
async downloadrpt(){
    let data_formmated=this.convertedData(this.rows);
    let date=new Date();
