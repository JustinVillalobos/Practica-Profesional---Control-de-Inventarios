...
export class BackupsComponent implements OnInit {
  ...
  restaurate() {
    if (this.data.length != 0) {
      this.AlertService.confirmacion(
        '¿Esta seguro/a de cargar un Backup?. Esta acción puede durar varios minutos',
        function (response, component) {
          if (response == true) {
            component.spinner.show();
            try {
              let edifices = component.converter.converterEdifices(
                component.data['Edifices']
              );
              let areas = component.converter.converterAreas(
                component.data['Areas']
              );
              let actives = component.converter.converterActives(
                component.data['Actives']
              );
              let loans = component.converter.converterLoan(
                component.data['Loans']
              );
              let areasactives = component.converter.converterAreasActive(
                component.data['Areas-Actives']
              );
              let loansactives = component.converter.converterLoanActive(
                component.data['Loans-Actives']
              );
              let data = {
                edifices: edifices,
                areas: areas,
