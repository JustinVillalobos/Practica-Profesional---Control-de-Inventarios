...
export class EditDistributionComponent implements OnInit {
  ...
  save(){
    ...
    if(this.amount == this.getCountByArea()){
      this.setAreas();
      this.spinner.show();
      this.ActiveService.editDistributionArea({"idActive":this.idActive,"areas":this.areas});
         electron.ipcRenderer.on("editDistributionActive", (event: any, data: any) => {
                  if(data["res"]){                               
                    this.spinner.hide();             
                   this.AlertService.alertTimeCorrect("Se ha generado una distribución del activo",function(_component){
