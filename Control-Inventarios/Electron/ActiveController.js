...
module.exports = class Active {
    ...
    async activesByIdArea(data) {
    	let active = [data];
    	 let res  = await this.promiseMethod('call sp_allActives_by_id_area(?)',active);
    	 return res;
    }
     async activesByIdEdifice(data) {
    	let active = [data];
    	 let res  = await this.promiseMethod('call sp_allActives_by_id_edifice(?)',active);
    	 return res;
    }

    async addActive(data) {

    	let active = [data.active.name,data.active.licensePlate,data.active.mark,data.active.model,data.active.serie,data.active.amount,data.active.placeOrigin,data.active.description];
    	 let res  = await this.promiseMethod('call sp_add_active(?,?,?,?,?,?,?,?)',active);
    	 return res;
    }
    async editActive(data) {
    	let active = [data.idActive,data.name,data.licensePlate,data.mark,data.model,data.serie,data.amount,data.placeOrigin,data.description];
    	 let res  = await this.promiseMethod('call sp_edit_active(?,?,?,?,?,?,?,?,?)',active);
    	 return res;
    }
    ...
    async insertLoanActive(data) {
    	let active = [data.idLoan,data.idActive];
    	 let res  = await this.promiseMethod('call sp_insert_loan_active(?,?)',active);
    	 return res;
    }
     ...
     async sp_delete_distribution(data) {
     	let active = [data];
     	let res  = await this.promiseMethod('call sp_delete_distribution(?)',active);
     }
    	...
    	let res ;
	    	for(let i=0;i<data.areas.length;i++){
	    		...
	    		active = [data.idActive,data.areas[i].idArea,data.areas[i].amount];
	    	 	res  = await this.promiseMethod('call sp_edit_distribution(?,?,?)',active);
	    	}
    	 return res;
    }
