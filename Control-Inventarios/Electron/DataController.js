...
module.exports = class Data {
	...
	async promiseMethod(consult,data){
	 ...
	 comprobarfaltantesCoincidentes(data,actives){
    	...
    	let res ;
	    	for(let i=0;i<data.length;i++){
	    	 	res  = actives.filter(response => response.licensePlate == data[i].licensePlate);
	    	 	if(res=="" || res==null || res.length==0){
	    	 		newFaltantes.push({"licensePlate":data[i].licensePlate,"name":data[i].name});
	    	 	}else{
	    	 		newCoincidentes.push({"licensePlate":data[i].licensePlate,"name":data[i].name});
	    	 	}
	    	}
	...
	comprobar(data,actives){
		...
		return {"Faltantes":res["Faltantes"],"Coincidentes":res["Coincidentes"],"Sobrantes":dataS};
	}
}